...
def image_to_base64(image_array):
    try:
        ...
        logger.debug(f"调整后的图像大小: {image.size}, 模式: {image.mode}")
...
def detect():
    try:
        ...
        # 构建输出路径
        output_dir = '/mnt/e/postgraduateStudy/graduation_thesis/SeaFog-DetSys/nc/detect-image'
        ...
        logger.info(f"Starting detection with parameters:")
        ...
        # 构建检测命令
        cmd = [
            '/home/tq/anaconda3/envs/mask2former/bin/python',
            'demo.py',
            '--config-file', config_path,
            '--input', image_path,
            '--output', output_path,
            '--opts',
            'MODEL.WEIGHTS', model_path,  # 将 MODEL.WEIGHTS 和路径分开
        ]
        
        # 执行检测命令
        process = subprocess.Popen(
            cmd,
            cwd=os.path.join(os.path.dirname(model_path), '../../demo'),
        ...
        logger.info(f"Detection completed successfully. Output file: {output_filename}")
        
        # 返回结果路径
        return jsonify({
            'resultPath': output_filename,

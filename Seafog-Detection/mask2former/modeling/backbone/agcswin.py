...
import numpy as np
import time
# from .agent_cswin import AgentAttention
...
class AgentCSWin(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=64, depth=[1,2,21,1], split_size = 7, la_split_size='96-48-24-12',
                 num_heads=[1,2,4,8], mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, use_chk=False,
        ...
        self.stage1 = nn.ModuleList([
            CSWinBlock(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate,
                drop_path=dpr[i], norm_layer=norm_layer,
                agent_num=int(agent_num[0]),attn_type=attn_types[i])
            for i in range(depth[0])])
        ...
        self.stage2 = nn.ModuleList(
            [CSWinBlock(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate,
                drop_path=dpr[np.sum(depth[:1])+i], norm_layer=norm_layer,
                agent_num=int(agent_num[1]),attn_type=attn_types[i])
            for i in range(depth[1])])
        ...
        temp_stage3.extend(
            [CSWinBlock(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate,
                drop_path=dpr[np.sum(depth[:2])+i], norm_layer=norm_layer,
                agent_num=int(agent_num[2]),attn_type=attn_types[i])
            for i in range(depth[2])])
        ...
        self.stage4 = nn.ModuleList(
            [CSWinBlock(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate,
                drop_path=dpr[np.sum(depth[:-1])+i], norm_layer=norm_layer, last_stage=True,
                agent_num=int(agent_num[3]),attn_type=attn_types[i])
            for i in range(depth[-1])])

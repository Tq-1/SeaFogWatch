...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses,
                 num_points, oversample_ratio, importance_sample_ratio,dn_no_lb=False):
    ...
    def forward(self, outputs, targets):
        ...
        if self.training and dn_out:
            dn_args=dn_out['dn_args']
            scalar=dn_args['pad_size']//dn_args['max_num']
            dn_out_without_aux={k: v for k, v in dn_out.items() if k != "aux_outputs"}
            dn_indices =[]
            for i in range(len(targets)):
                if len(targets[i]['labels']) > 0:
                    ...
                    tgt_idx = t.flatten()
                    output_idx = (torch.tensor(range(scalar)) * dn_args['max_num']).long().cuda().unsqueeze(1) + t
            ...
            for loss in self.losses:
                l_dict.update(
                    self.get_loss(loss, dn_out_without_aux, targets, dn_indices, num_masks * scalar))

            l_dict = {k + f'_dn': v for k, v in l_dict.items()}
            losses.update(l_dict)
        else:
            # 'labels_exc', 'boxes_exc', 'cardinality_exc'
            l_dict = dict()
            l_dict['loss_mask_dn'] = torch.as_tensor(0.,device=next(iter(outputs.values())).device)
            l_dict['loss_dice_dn'] = torch.as_tensor(0.,device=next(iter(outputs.values())).device)
            l_dict['loss_ce_dn'] = torch.as_tensor(0.,device=next(iter(outputs.values())).device)
            losses.update(l_dict)
        ...
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                # if i>0:
                if self.training and dn_out:
                    aux_outputs_known = dn_out['aux_outputs'][i]
                    l_dict={}

                    for loss in self.losses:
                        l_dict.update(self.get_loss(loss, aux_outputs_known, targets, dn_indices, num_masks*scalar))
                    ...
                    losses.update(l_dict)
        if self.dn_no_lb:
            losses={k:losses[k] for k in losses if not k.startswith("loss_ce_dn")}
